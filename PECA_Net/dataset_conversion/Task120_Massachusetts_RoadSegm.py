...
from PECA_Net.utilities.file_conversions import convert_2d_image_to_nifti
...
if __name__ == '__main__':
    ...
    `PECA_Net_plan_and_preprocess` command like this:
    
    > PECA_Net_plan_and_preprocess -t 120 -pl3d None
    
    once that is completed, you can run the trainings as follows:
    > PECA_Net_train 2d PECA_NetTrainerV2 120 FOLD
    
    (where fold is again 0, 1, 2, 3 and 4 - 5-fold cross validation)
    
    there is no need to run PECA_Net_find_best_configuration because there is only one model to shoose from.
    Note that without running PECA_Net_find_best_configuration, nnU-Net will not have determined a postprocessing
    ...
    `PECA_Net_determine_postprocessing` command
    """
