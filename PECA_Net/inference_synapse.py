...
def test(fold):
    path1='H:/CT/Codes_of_CT/PECA_Net-main/DATASET/PECA_Net_raw/PECA_Net_raw_data/Task02_Synapse/labelsTs'
    path2='H:/CT/results/synapse/IB_N_scSE/fold_3/model_best'
    label_list=sorted(glob.glob(os.path.join(path1,'*nii.gz')))
    infer_list=sorted(glob.glob(os.path.join(path2,'*nii.gz')))
