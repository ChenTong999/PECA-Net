...
from PECA_Net.inference.segmentation_export import save_segmentation_nifti_from_softmax, save_segmentation_nifti
...
from PECA_Net.postprocessing.connected_components import load_remove_save, load_postprocessing
from PECA_Net.training.model_restore import load_model_and_checkpoint_files
...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None):
    ...
    assert isinstance(trainer, PECA_NetTrainer) or isinstance(trainer, PECA_NetTrainer_synapse)
    q = Queue(1)
    processes = []
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
                        ...
                        step_size: float = 0.5, checkpoint_name: str = "model_best",  # model_best  model_final_checkpoint
                        segmentation_export_kwargs: dict = None):
    """
