...
from PECA_Net.configuration import default_num_threads
from PECA_Net.experiment_planning.DatasetAnalyzer import DatasetAnalyzer
from PECA_Net.experiment_planning.common_utils import split_4d_nifti
from PECA_Net.paths import PECA_Net_raw_data, PECA_Net_cropped_data, preprocessing_output_dir
from PECA_Net.preprocessing.cropping import ImageCropper
...
def split_4d(input_folder, num_processes=default_num_threads, overwrite_task_output_id=None):
    ...
    output_folder = join(PECA_Net_raw_data, "Task%03.0d_" % overwrite_task_output_id + task_name)
    ...
    # maybe_mkdir_p(output_folder)  # maybe_mkdir_p()没有起作用，用自定义函数mkdir_func()代替
    mkdir_func(output_folder)
...
def create_lists_from_splitted_dataset(base_folder_splitted):
    ...
    for tr in training_files:
        ...
        for mod in range(num_modalities):
            cur_pat.append(join(base_folder_splitted, "imagesTr", tr['image'].replace('\\', '/').split("/")[-1][:-7] +
                                "_%04.0d.nii.gz" % mod))
        cur_pat.append(join(base_folder_splitted, "labelsTr", tr['label'].replace('\\', '/').split("/")[-1]))
...
def crop(task_string, override=False, num_threads=default_num_threads):
    cropped_out_dir = join(PECA_Net_cropped_data, task_string)
    # maybe_mkdir_p(cropped_out_dir)
    mkdir_func(cropped_out_dir)
    ...
    splitted_4d_output_dir_task = join(PECA_Net_raw_data, task_string)
    lists, _ = create_lists_from_splitted_dataset(splitted_4d_output_dir_task)

    imgcrop = ImageCropper(num_threads, cropped_out_dir)
    imgcrop.run_cropping(lists, overwrite_existing=override)
    shutil.copy(join(PECA_Net_raw_data, task_string, "dataset.json"), cropped_out_dir)
...
def plan_and_preprocess(task_string, processes_lowres=default_num_threads, processes_fullres=3, no_preprocessing=False):
    from PECA_Net.experiment_planning.experiment_planner_baseline_2DUNet import ExperimentPlanner2D
    from PECA_Net.experiment_planning.experiment_planner_baseline_3DUNet import ExperimentPlanner
    ...
    mkdir_func(preprocessing_output_dir_this_task_train)

    shutil.copy(join(cropped_out_dir, "dataset_properties.pkl"), preprocessing_output_dir_this_task_train)
    shutil.copy(join(PECA_Net_raw_data, task_string, "dataset.json"), preprocessing_output_dir_this_task_train)
