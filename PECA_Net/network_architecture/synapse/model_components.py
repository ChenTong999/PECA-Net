...
from PECA_Net.network_architecture.layers import LayerNorm
from PECA_Net.network_architecture.synapse.transformerblock import TransformerBlock as PIM
from PECA_Net.network_architecture.dynunet_block import get_conv_layer, UnetResBlock
    ...
    def __init__(self, input_size=[32 * 32 * 32, 16 * 16 * 16, 8 * 8 * 8, 4 * 4 * 4],dims=[32, 64, 128, 256],
        ...
        for i in range(4):
            ...
            for j in range(depths[i]):
                stage_blocks.append(PIM(input_size=input_size[i], hidden_size=dims[i],  proj_size=proj_size[i],
                                        num_heads=num_heads, dropout_rate=transformer_dropout_rate, pos_embed=True))
    ...
    def     __init__(
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                stage_blocks.append(PIM(input_size=out_size, hidden_size=out_channels, proj_size=proj_size,
                                        num_heads=num_heads, dropout_rate=0.15, pos_embed=True))
