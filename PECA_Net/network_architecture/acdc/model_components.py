...
from PECA_Net.network_architecture.acdc.transformerblock import TransformerBlock as PIM
...
class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[16 * 40 * 40, 8 * 20 * 20, 4 * 10 * 10, 2 * 5 * 5], dims=[32, 64, 128, 256],
                 proj_size=[256,256,256,256], depths=[3, 3, 3, 3],  num_heads=4, spatial_dims=3, in_channels=1,
        ...
        for i in range(4):
            ...
            for j in range(depths[i]):
                stage_blocks.append(PIM(input_size=input_size[i], hidden_size=dims[i],
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                stage_blocks.append(PIM(input_size=out_size, hidden_size=out_channels, proj_size=proj_size,
                                        num_heads=num_heads, dropout_rate=0.1, pos_embed=True))
